...
MTCNN::MTCNN(const string &model_path) 
{
	...
	faceQual = new FaceQuality(false);
    //faceQual->loadModel(pose_model);
...
void MTCNN::detect(cv::Mat& img_, std::vector<Bbox>& finalBbox_)
{
	...
	printf("Time cost:Max %.2fms,Min %.2fms,Avg %.2fms\n", max_time,min_time,total_time/count);
#endif

}


void MTCNN::detectMaxFace(cv::Mat& img_, std::vector<Bbox>& finalBbox) 
{
...
void MTCNN::detect(cv::Mat& img, /*int minsize,*/ float scale,std::vector<FaceBox>& faceBoxs/*, int num_threads*/)
{
	this->scale = scale;
//	this->minsize = minsize;
//	this->num_threads = num_threads;
	std::vector<Bbox>allFaces;
	detect(img, allFaces);
	for(int f = 0; f < allFaces.size(); f++)
	{
		...
		{
			Point2f p;
			p.x = allFaces[f].ppoint[l] / scale;
			p.y = allFaces[f].ppoint[l + 5] / scale;
			fb.lmk.push_back(p);
		}
		fb.score = allFaces[f].score;
		fb.bbox.x = allFaces[f].x1 / scale;
		fb.bbox.y = allFaces[f].y1 / scale; 
		fb.bbox.width  = (allFaces[f].x2 - allFaces[f].x1) / scale;
		fb.bbox.height = (allFaces[f].y2 - allFaces[f].y1) / scale;
		FaceParams faceParams = faceQual->checkQuality(img, fb.bbox, fb.lmk);
		fb.rot_x = faceParams.rot_x;
		fb.rot_y = faceParams.rot_y;
		fb.rot_z = faceParams.rot_z;
		faceBoxs.push_back(fb);
	}
}

void MTCNN::detectMaxFace(cv::Mat& img, int minsize, float scale, std::vector<FaceBox>& faceBoxs, int num_threads)
{
	this->scale = scale;
    this->minsize = minsize;
    this->num_threads = num_threads;
	std::vector<Bbox>allFaces;
	detectMaxFace(img, allFaces);
	for(int f = 0; f < allFaces.size(); f++)
	{
		FaceBox fb;
		for(int l = 0; l < 5; l++)
